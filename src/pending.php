...
<h1 align="center">Pending Tickets</h1>
</div>
</div>
</div>
</div>
...
<?php
	$count=1;
	$qry="select *,(select user_name from tbl_user where user_id=tbl_ticket.user_id) as uname,(select product_name from tbl_products where product_id=tbl_ticket.product_id) as pname from tbl_ticket where status='assign' and emp_id='".$_SESSION['eid']."'";
	$rs = mysql_query($qry);
...
<td class="product-subtotal text-center"><?php echo $pdate; ?></td>
