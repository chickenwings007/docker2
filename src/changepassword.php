...
{
	$qry = mysql_query("select * from tbl_user where user_id ='". $_SESSION['uid'] ."' and password='".$_REQUEST['txtoldpass']."'") or die(mysql_error());
	if(mysql_num_rows($qry)>0)
	{
		...
		if($pass == $_REQUEST['txtoldpass'])
		{
			...
			$sql=mysql_query("update tbl_user set password='".$_REQUEST['txtpassword']."' where user_id='". $_SESSION['uid'] ."'");
			if($sql)
			{
