...
if (isset($_POST["hfAction"]) && $_POST["hfAction"] != "") {
    ...
    if ($_POST["hfAction"] == "add") {
        ...
        $sql = "";        
        if ($id == "") {
            $sql = "insert into tbl_subcategory values(NULL,'" . $_REQUEST['txtcatname'] . "','".$_REQUEST['txtsubcatname']."','" . mysql_real_escape_string($_REQUEST['txtsubcatdes']) . "','".$filename."')";
        } else {
		if ($_FILES['txtsubcatimg']['name'] != "") 
		{
            ...
            $sql = "update tbl_subcategory set cat_id='".mysql_real_escape_string($_REQUEST['txtcatname'])."',subcat_name='".mysql_real_escape_string($_REQUEST['txtsubcatname'])."',subcat_desc='" . mysql_real_escape_string($_REQUEST["txtsubcatdes"]) . "',subcat_img='" . $filename . "' where subcat_id='" . $id . "'";
        }
    ...
    if ($action == "getdata") {
        ...
        $id = $mydata[0]->id;
        $img = mysql_query("select subcat_img from tbl_subcategory where subcat_id='" . $id . "'");
