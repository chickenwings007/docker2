...
if (isset($_POST["hfAction"]) && $_POST["hfAction"] != "") {
    ...
    if ($_POST["hfAction"] == "add") {
        ...
        $sql = "";        
        if ($id == "") {
            $sql = "insert into tbl_employee values(NULL,'" . $_REQUEST['txtempname'] . "','".$_REQUEST['txtcontact']."','".$_REQUEST['txtpassword']."','".$filename."','". $_REQUEST['txtemail'] ."','". mysql_real_escape_string($_REQUEST['txtempadd']) ."','". $_REQUEST['r_block'] ."',NULL)";
        } else {
            ...
            $sql = "update tbl_employee set emp_name='".mysql_real_escape_string($_REQUEST['txtempname'])."',emp_contact='".mysql_real_escape_string($_REQUEST['txtcontact'])."',emp_img='" .$filename . "',emp_email='" . mysql_real_escape_string($_REQUEST["txtemail"]) . "',emp_address='" . mysql_real_escape_string($_REQUEST["txtempadd"]) . "',emp_block='" . mysql_real_escape_string($_REQUEST["r_block"]) . "' where emp_id='" . $id . "'";
        }
    ...
    if ($action == "getdata") {
        ...
        $id = $mydata[0]->id;
        $img = mysql_query("select emp_img from tbl_employee where emp_id='" . $id . "'");
