...
if(isset($_REQUEST["subcatid"]))
{
	...
	$sql=mysql_query
	("select *,(select subcat_name from tbl_subcategory where subcat_id=tbl_products.subcat_id) as subname,(select cat_name from tbl_category where cat_id=(select cat_id from tbl_subcategory where subcat_id=tbl_products.subcat_id)) as catname from tbl_products where subcat_id='". $ss ."'");
...
//	$sql=mysql_query
//	("select *,(select subcat_name from tbl_subcategory where subcat_id=tbl_products.subcat_id) as subname,(select cat_name from tbl_category where cat_id=(select cat_id from tbl_subcategory where subcat_id=tbl_products.subcat_id)) as catname from tbl_products where subcat_id='". $suub ."'");
//	if(mysql_num_rows($sql)<=0)
//	{
//		echo "<script>window.location='login.php';<script>";
//	}  
...
<p class="shop-result-count">Showing <?php echo $page; ?>&ndash;<?php echo $pageLimit; ?> of <?php $qry=mysql_query("select * from tbl_products"); echo mysql_num_rows($qry);?> results</p>
...
<div>
<input name="txtrating" id="txtrating" class="ratingEvent rating5" readonly="" value="<?php $a = mysql_query("select max(ratings) as rat from tbl_rating where product_id = '". $row["product_id"] ."'"); while($r = mysql_fetch_array($a)){ echo $r['rat']; } ?>"/>
</div>
...
<input type="text" id="min_price" name="txtminprice" value="" data-min="<?php $a = mysql_query("select min(price) as abgprice from tbl_products"); 
while($r = mysql_fetch_array($a))
{ 
echo $r['abgprice']; 
}
 ?>" placeholder="Min price"/>
<input type="text" id="max_price" name="txtmaxprice" value="" data-max="<?php $a = mysql_query("select max(price) as abgprice from tbl_products"); while($r = mysql_fetch_array($a)){ echo $r['abgprice']; } ?>" placeholder="Max price"/>
...
<li><a href="shop.php?subcatid=<?php echo $converter->encode($sub); ?>"><?php echo $row['subcat_name']; ?></a> <span class="count"><strong>(<?php $res=mysql_query("select * from tbl_products where subcat_id = '".  $sub ."'"); echo mysql_num_rows($res); ?>)</strong></span></li>
