...
if(isset($_REQUEST["sid"]))
{
	...
	$sql=mysql_query
	("select *,(select subcat_name from tbl_subcategory where subcat_id=tbl_products.subcat_id) as subname,(select cat_name from tbl_category where cat_id=(select cat_id from tbl_subcategory where subcat_id=tbl_products.subcat_id)) as catname from tbl_products where product_id ='".$id."'");
...
if(isset($_REQUEST["subcid"]))
{
	...
	$sql=mysql_query
	("select *,(select subcat_name from tbl_subcategory where subcat_id = tbl_products.subcat_id) as sub, (select cat_name from tbl_category where cat_id=(select cat_id from tbl_subcategory where subcat_id=tbl_products.subcat_id)) as cat from tbl_products where subcat_id = '". $subc ."'");
	if(mysql_num_rows($sql)<=0)
	{
		echo "<script>window.location='login.php';</script>";
	}  
...
(<span class="count"><?php $qry=mysql_query("select * from tbl_rating where product_id='". $converter->decode($_REQUEST["sid"]) ."'"); echo mysql_num_rows($qry); ?></span> customer reviews)
</a>
</div>
...
<li>
<a data-toggle="tab" role="tab" href="#tab-reviews">Reviews (<?php $qry=mysql_query("select * from tbl_rating where product_id='". $converter->decode($_REQUEST["sid"]) ."'"); echo mysql_num_rows($qry); ?>)</a>
</li>
</ul>
...
<?php
$qry = mysql_query("select *,(select subcat_name from tbl_subcategory where subcat_id = tbl_products.subcat_id) as sub, (select cat_name from tbl_category where cat_id=(select cat_id from tbl_subcategory where subcat_id=tbl_products.subcat_id)) as cat from tbl_products where subcat_id = '". $subc ."'");
